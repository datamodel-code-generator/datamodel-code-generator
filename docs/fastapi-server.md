...
datamodel-codegen --input openapi.yaml --input-file-type openapi \
  ...
  --output models.py --generate-server fastapi --target-config fastapi.toml
```
...
the existing files instead of writing them, and reports `E_MODEL_MISMATCH` when they differ.
