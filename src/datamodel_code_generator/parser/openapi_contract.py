...
@dataclass(slots=True)
class ModuleImportFrame:
    """Retain completed import returns without keeping model or parser owners alive."""

    values: dict[int, tuple[Import, ...]]
    failure: BindingCaptureError | None = None


_module_import_frame: ContextVar[ModuleImportFrame | None] = ContextVar("dcg_binding_module_imports", default=None)
...
@dataclass(slots=True)
class ConstrainedBranchFrame:
    """Borrow the actual materialized branch while its existing root producer runs."""
...
class BindingCaptureMixin(OpenAPIParser):
    ...
    def __init__(  # ruff: ignore[too-many-statements] -- Initialize all attempt-owned capture state together.
        ...
        **options: Unpack[OpenAPIParserConfigDict],
        ...
        self._constrained_branches: list[ConstrainedBranchFrame] = []
        ...
        self._module_import_sources: dict[int, tuple[Import, ...]] = {}
    ...
    def _record_schema_type(self, schema: JsonSchemaObject, result: DataType) -> DataType:
        partial = any(schema is branch for frame in self._constrained_branches for branch in frame.schemas.values())
        self.schema_types.append(SchemaTypeObservation(schema, result, partial=partial))
        return result
    ...
    def _parse_additional_properties_value(
        ...
        *,
        ...
        start = len(self.binding_ledger.registrations)
        ...
        self._record_created_root(additional_properties, result, start)  # pyright: ignore[reportUnknownArgumentType]
    ...
    def _parse_constrained_type_union(self, name: str, obj: JsonSchemaObject, path: list[str]) -> DataType | None:
        """Connect object-key aliases created by the actual heterogeneous union producer."""
        if not isinstance(obj.type, list):
            return super()._parse_constrained_type_union(name, obj, path)  # pyright: ignore[reportUnknownMemberType, reportUnknownVariableType]
        frame = ConstrainedBranchFrame(obj, {})
        self._constrained_branches.append(frame)
        start = len(self.binding_ledger.registrations)
        try:
            result: DataType | None = super()._parse_constrained_type_union(name, obj, path)  # pyright: ignore[reportUnknownMemberType, reportUnknownVariableType]
        finally:
            self._constrained_branches.pop()
        self._record_constrained_union(frame, result, start)  # pyright: ignore[reportUnknownArgumentType]
        return result  # pyright: ignore[reportUnknownVariableType]

    @capture_errors
    def _record_constrained_union(self, frame: ConstrainedBranchFrame, result: DataType | None, start: int) -> None:
        if result is None or not frame.schemas:
            return
        source_types: str | list[str] | None = frame.source.type
        if not isinstance(source_types, list) or len(
            types := tuple(item for item in source_types if item != "null")
        ) != len(result.data_types):
            msg = "A constrained union has unmatched branch returns"
            raise BindingCaptureError(msg)
        for type_, branch in zip(types, result.data_types, strict=True):
            if (schema := frame.schemas.get(type_)) is not None:
                self._record_created_root(schema, branch, start)
    ...
    def _parse_array_union_constrained_branch(
        ...
        """Observe constrained aliases without changing the guarded root parser."""
        frame = ConstrainedBranchFrame(obj, {})
        self._constrained_branches.append(frame)
        start = len(self.binding_ledger.registrations)
        try:
            result: DataType = super()._parse_array_union_constrained_branch(  # pyright: ignore[reportUnknownMemberType, reportUnknownVariableType]
                name, obj, path, type_, fallback_data_type
            )
        finally:
            self._constrained_branches.pop()
        self._record_constrained_branch(frame, type_, result, start)  # pyright: ignore[reportUnknownArgumentType]
        return result  # pyright: ignore[reportUnknownVariableType]

    def _get_array_union_branch_schema(self, obj: JsonSchemaObject, type_: str) -> JsonSchemaObject:
        """Retain the actual branch clone, including its original nested schema nodes."""
        result: JsonSchemaObject = super()._get_array_union_branch_schema(obj, type_)  # pyright: ignore[reportUnknownMemberType, reportUnknownVariableType]
        return self._record_array_union_schema(obj, type_, result)  # pyright: ignore[reportUnknownArgumentType]

    @capture_errors
    def _record_array_union_schema(
    ...
    @capture_errors
    def _record_constrained_branch(
        self, frame: ConstrainedBranchFrame, type_: str, result: DataType, start: int
    ) -> None:
        if (schema := frame.schemas.get(type_)) is None:
            msg = "A constrained union has no observed branch schema"
            raise BindingCaptureError(msg)
        self._record_created_root(schema, result, start)
    ...
    def _record_field_construction(  # ruff: ignore[too-many-arguments]
        ...
        *,
        ...
        if schema is not None:
            self._record_schema_type(schema, field.data_type)
        self.field_constructions[identity] = FieldConstructionObservation(
            identity,
            schema,
            required,
    ...
    def _parse_schema_or_ref(self, name: str, schema: MediaSchema, path: list[str]) -> DataType:
        ...
        self._record_created_root(schema, result, registrations)  # pyright: ignore[reportUnknownArgumentType]
    ...
    @capture_errors
    def _record_created_root(self, schema: MediaSchema, result: DataType, start: int) -> None:
    ...
    def _finalize_modules(
        ...
        """Observe original imports around the single ordinary finalization and remapping."""
        global_sources = self._record_global_imports()
        frame = ModuleImportFrame({})
        token = _module_import_frame.set(frame)
        try:
            super()._finalize_modules(contexts, unused_models, model_to_module_models, module_to_import)  # pyright: ignore[reportUnknownMemberType]
            self._record_module_import_sources(contexts, frame.values, global_sources)
        finally:
            _module_import_frame.reset(token)
            if frame.failure is not None:
                self.binding_ledger.remember_failure(frame.failure)
                frame.failure = None
            frame.values.clear()

    @capture_errors
    def _record_global_imports(self) -> tuple[Import, ...]:
        return freeze_imports((self.imports,))

    @capture_errors
    def _record_module_import_sources(
        self,
        contexts: list[ModuleContext],
        model_imports: dict[int, tuple[Import, ...]],
        global_sources: tuple[Import, ...],
    ) -> None:
        """Corroborate actual producer returns with imports that survived in their own module."""
        for ctx in contexts:
            resolver = FinalImportResolver(freeze_imports((self.imports, ctx.imports)), self.config.import_overrides)
            sources = dict.fromkeys(global_sources)
            sources.update((source, None) for model in ctx.models for source in model_imports.get(id(model), ()))
            adopted: list[Import] = []
    ...
    def _record_module_output(self, ctx: ModuleContext, result: Result | None) -> Result | None:
        if result is not None:
            self.module_outputs.append(
                ModuleOutputObservation(
                    ctx.module,
                    tuple(ctx.models),
                    result,
                    (self.imports, ctx.imports),
                    self._module_import_sources.get(id(ctx.imports), ()),
                )
            )
        return result
    ...
    def _release_capture(self) -> None:
        ...
        self._constrained_branches.clear()
        ...
        self._module_import_sources.clear()
