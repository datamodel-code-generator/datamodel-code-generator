...
# Source: pydantic_v2/schema_runtime_validation.jinja2 (sha256:761d63ec194fe251)
...
def render_with_scope(_scope: _Scope) -> str:
    ...
    if _context_schema_runtime_validation:
        ...
        if any_of_required_groups:
            ...
            _parts.append(f"\n    )\n{_value_15}")
            _setattr(schema_validator_state, "has_prior", True)
        not_required_groups = _filter_list(
            _filter_selectattr(
                _getattr(_context_schema_runtime_validation, "required_groups"), "keyword", "equalto", "not"
            )
        )
        if not_required_groups:
            if _getattr(schema_validator_state, "has_prior"):
                _value_16 = _stringify("\n")
                _parts.append(f"\n{_value_16}")
            _parts.append("\n    __json_schema_not_required_groups__: ClassVar[tuple[Any, ...]] = (")
            for rule in not_required_groups:
                _value_17 = _stringify(_filter_repr(getattr(rule, "groups", _MISSING)))
                _parts.append(f"\n        {_value_17},")
            _value_18 = _stringify("\n")
            _parts.append(f"\n    )\n{_value_18}")
            _setattr(schema_validator_state, "has_prior", True)
        if _getattr(_context_schema_runtime_validation, "conditional_required"):
            if _getattr(schema_validator_state, "has_prior"):
                _value_19 = _stringify("\n")
                _parts.append(f"\n{_value_19}")
            ...
            for rule in _getattr(_context_schema_runtime_validation, "conditional_required"):
                _value_20 = _stringify(_filter_repr(getattr(rule, "condition", _MISSING)))
                _value_21 = _stringify(_filter_repr(getattr(rule, "then_groups", _MISSING)))
                _value_22 = _stringify(_filter_repr(getattr(rule, "else_groups", _MISSING)))
                _parts.append(
                    f"\n        {{\n            'condition': {_value_20},\n            'then_required_groups': {_value_21},\n            'else_required_groups': {_value_22},\n        }},"
                )
