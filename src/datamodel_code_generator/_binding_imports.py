...
def freeze_imports(collections: tuple[Imports, ...]) -> tuple[Import, ...]:
    """Read actual import storage without invoking model getters or rendering again."""
...
class FinalImportResolver:
    ...
    def __init__(
        ...
        *,
        emitted_sources: tuple[Import, ...] | None = None,
    ) -> None:
        ...
        self._emitted_sources = (
            frozenset((value.from_, value.import_, value.keep_unaliased) for value in emitted_sources)
    ...
    def resolve(self, import_: Import) -> Import:
        """Keep the actual emitted module/alias; operation-only producers keep their own unremapped import."""
        if (resolved := self._cache.get(import_)) is not None:
            return resolved
        if (
            self._emitted_sources is not None
            and (import_.from_, import_.import_, import_.keep_unaliased) not in self._emitted_sources
        ):
            self._cache[import_] = import_
            return import_
    ...
    def _type(  # ruff: ignore[too-many-return-statements]
        ...
        match value:
            ...
            case ConstructorType(callable_, keywords):
                callee = ImportedType(self.resolve(callable_.import_), callable_.qualified_suffix)
