...
## 5. Server handlers, authentication, and FastAPI settings
...
Handler-signature Protocols have each operation's fixed keywords and the return type above. Sync/async have separate Protocols; async uses `async def __call__(...) -> Result`. Only Protocols receiving a principal have `PrincipalT_contra`; operations with an anonymous alternative accept `PrincipalT | None`, while unauthenticated operations have no principal argument at all. `Handlers[PrincipalT]` is a class-syntax generic TypedDict whose required keys are the **fixed Python handler-function names** of all selected operations, each value being that mode/operation's Protocol. Use FASTAPI's unique non-keyword identifier naming rules: for example, the key is `get_pet`, while the OperationKey for operation_dependencies/install_openapi/selection remains `/paths/~1pets/get`. Preserve their mapping table and handler-reuse collision guard. Do not use pointers or original scheme names as invalid generic-TypedDict field names. Do not weaken the typed builder entry point back to `Mapping[str, Callable]`. Retain startup checks for dynamic-input missing/unknown keys/callability and similar properties, without treating those checks alone as proof of return types.
...
|`Authorizer[SecretT_contra, PrincipalT_co]`|`__call__(context: AuthContext[SecretT_contra], /) -> PrincipalT_co`. AsyncAuthorizer is async def with the same result type. Rejection uses existing HTTPException. Preserve separate axes for AuthContext's opaque type and the principal type received by handlers|
...
For `build_router` / `create_app`, handlers is `Handlers[PrincipalT] | None`, authorizer is `Authorizer[SecretT, PrincipalT] | AsyncAuthorizer[SecretT, PrincipalT] | None`, credential_extractors is `CredentialExtractors[SecretT] | None`, and operation_dependencies is `OperationDependencies | None`. Preserve other existing arguments and returns. Statically reject incompatible principals between handlers/authorizers, and include a positive case for an authorizer correctly handling a SecretT union. An unused principal argument in an unimplemented scaffold may be recorded as a boundary accepting object, without fixing user implementations' principal types to object.

The generator cannot know the user's Principal type, so do not generate automatic conformance assignments such as `OpHandler[object] = actual_handler` for principal-bearing handlers. Preserve existing assignments for operations without principals. The standard README path with a principal has users pass `Handlers[Principal]` and matching `Authorizer[Secret, Principal]` into the same builder, and statically checks this actual code. Users may also assign to a concrete `OpHandler[Principal]` in their own type-checking module. Implicit scaffold import through `handlers=None` is a dynamic connection: validate signatures/keys/modes at startup without guaranteeing static proof of principal correlation. Do not switch typing rules based on whether a scaffold was edited, or omit ordinary strict checking of the scaffold .py itself.
...
|TY08|Handlers' Python-name keys and operation signatures, sync/async, and authorizer SecretT/PrincipalT correlations. Incorrect principals/extractor results/undeclared handlers are negative cases. Do not confuse keys with OperationKey pointers. Check explicit typed connections for principal-bearing handlers without extending static guarantees to handlers=None. Check scaffold .py itself rather than hiding behind .pyi|
