...
| [FASTAPI](DECISIONS-FASTAPI.md) | All 13 workflows, routes/handlers, sync/async, router updates, forms/uploads, security, served OpenAPI, templates/hooks |
...
- A mandatory HandlerProtocol selection mode, async-only handlers, a fixed seven-file output, JSON-only/flat-type coverage, or no-reuse/single-module restrictions presented as a complete replacement.
...
Preserve handwritten handlers during server regeneration. Check the actual handler.py against Protocols representing current signatures. Independently test new required arguments, type changes, sync/async changes, operation deletion, and explicit renaming; adjacent `.pyi` files must not conceal implementation checking. Pass user-owned typed `Handlers[Principal]` and the authorizer to the same builder for static checking. Do not infer handwritten types or treat handlers=None dynamic wiring as proof of static compatibility. Apply all relevant TY oracles to new generated artifacts.
