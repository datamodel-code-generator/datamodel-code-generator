...
- Native union selection is Pydantic's (smart or discriminated). The schema-based choice only drives the walk, extras and error pointers, so a union whose members overlap on the wire can construct a member the schema did not pick, for example an `extra='forbid'` member next to one that ignores extras.
