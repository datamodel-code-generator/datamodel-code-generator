...
Business logic is user-owned and lives outside the generated package, in classes that implement the generated service Protocols or in any objects with the same methods. The generator owns every file it writes, rewrites them on each run, and never overwrites or merges handwritten code. New, removed, renamed, or changed operations change the Protocols, so type checkers, Python's refusal to instantiate a subclass that lacks an abstract method, and the builders' startup checks report the implementations to update. When authentication uses a concrete user Principal type, implement `<Group>Service[Principal]` and pass the corresponding authorization function to the same builder for checking. Do not present startup argument-shape checks as proof of all types. See the [regeneration contract](DECISIONS-FASTAPI.md).
