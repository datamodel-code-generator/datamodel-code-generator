...
def producer_failure(source: Path, case: dict[str, str], monkeypatch: pytest.MonkeyPatch) -> dict[str, object]:
    ...
    with producer_fault(case, monkeypatch):
        ...
        actual, retained = run_generation_session(
            source,
            document=document,
            field_constraints=case.get("field_constraints") == "true",
            api_scope=case.get("api") == "true",
            allof_merge_mode=AllOfMergeMode.NoMerge if case.get("no_merge") == "true" else AllOfMergeMode.Constraints,
        )
    return {
        "error": actual["error"],
        "batch": actual["batch"],
