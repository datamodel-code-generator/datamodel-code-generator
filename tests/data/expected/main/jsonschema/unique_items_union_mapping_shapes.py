...
#   filename:  unique_items_union_mapping_shapes.json
...
class _JsonSchemaRuntimeValidationBase(BaseModel):
    ...
    def _validate_json_schema_unique_items_path(
        ...
        match path[position]:
            ...
            case '__json_schema_mapping_values__':
                if not (isinstance(value, dict) or isinstance(value, _collections_abc.Mapping)):
                    return
    ...
    def _json_schema_unique_items_equal(cls, left: Any, right: Any) -> bool:
        match left:
            ...
            case dict() | _collections_abc.Mapping() as object_:
                if not (
                    isinstance(right, dict) or isinstance(right, _collections_abc.Mapping)
...
class OneOfFallback(BaseModel):
    kind: str


class MappingEnumScalar(Enum):
    scalar = 'scalar'
    int_0 = 0


class MappingEnumLiteral(Enum):
    value_0 = {'x': [1, 1]}
...
class UniqueItemsUnionMappingShapes(_JsonSchemaRuntimeValidationBase):
    __json_schema_unique_items__: ClassVar[tuple[tuple[object, ...], ...]] = (
        ...
        (('arrayConstScalar',),),
        (('mappingEnumScalar',), '__json_schema_mapping_values__'),
    )

    anyOfFallback: Any | dict[str, Any]
    oneOfFallback: Any | OneOfFallback
    anyOfMappingOnly: Any | str
    arrayImpossibleBranch: list[Any] | list[int]
    mappingImpossibleBranch: list[Any] | dict[str, list[int]]
    arrayConstScalar: list[int] | Literal['scalar']
    mappingEnumScalar: dict[str, list[int]] | MappingEnumScalar
    arrayConstLiteral: list[int]
    mappingEnumLiteral: dict[str, list[int]] | MappingEnumLiteral