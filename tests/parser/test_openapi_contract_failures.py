...
from tests.data.python.binding_capture_failures import producer_failure

DATA = Path(__file__).parents[1] / "data"
SOURCE = DATA / "generation_platform/binding"
EXPECTED = DATA / "expected/main/generation_platform/binding/producer-failures"
...
def test_inconsistent_producer_observation(case: dict[str, str], monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    actual = producer_failure((SOURCE / case["source"]).resolve(), case, monkeypatch)
