...
@pytest.mark.parametrize(
    ...
    [
        pytest.param({"allOf": [False, {"type": "integer"}]}, id="direct-false"),
        pytest.param(
            {"allOf": [{"$ref": "#/$defs/Never"}, {"type": "integer", "minimum": 1}]},
            id="ref-false",
        ),
    ],
)
def test_main_jsonschema_additional_properties_allof_false_schema_errors(
    additional_properties: dict[str, object], output_file: Path
) -> None:
    """Reject false allOf branches while preserving annotated mapping constraints."""
    _run_jsonschema_dict(
        {
            "title": "Payload",
            "type": "object",
...
_ADDITIONAL_PROPERTIES_CONSTRAINTS_VALID_JSON = (
    ...
    '"localTitleRefMap":{"ok":2},'
    '"namedMap":{"ok":5},'
    '"rootMap":{"ok":2},"nullableValueMap":{"nil":null,"ok":1},'
    '"closedArrayValueMap":{"empty":[]},"allOfValueMap":{"ok":5},'
    '"titledAllOfValueMap":{"ok":5},'
    '"nullableAllOfValueMap":{"nil":null,"ok":5},'
    '"unionValueMap":{"int":5,"text":"good"},'
    '"objectValueMap":{"entry":{"key":"value"}},'
    '"modeledValueMap":{"entry":{"value":1}},"titledValueMap":{"ok":2},'
    '"modeledIrrelevantConstraintMap":{"entry":{"value":"ok"}},'
    '"arrayIrrelevantConstraintMap":{"entry":[1]},'
    '"heterogeneousValueMap":{"int":1,"text":"ok"},'
    '"booleanIrrelevantConstraintMap":{"flag":true},"extra":3}'
)
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            ...
            "additional_properties_value_constraints_annotated_pydantic_v2.py",
            id="pydantic-v2",
        ),
        pytest.param(
            DataModelType.PydanticV2Dataclass,
            "additional_properties_value_constraints_annotated_pydantic_v2_dataclass.py",
            id="pydantic-v2-dataclass",
        ),
        pytest.param(
            DataModelType.MsgspecStruct,
            "additional_properties_value_constraints_annotated_msgspec.py",
            id="msgspec",
        ),
        pytest.param(
            DataModelType.DataclassesDataclass,
            "additional_properties_value_constraints_annotated_dataclass.py",
            id="dataclass",
        ),
        pytest.param(
            DataModelType.TypingTypedDict,
            "additional_properties_value_constraints_annotated_typed_dict.py",
            id="typed-dict",
        ),
    ],
)
def test_main_jsonschema_additional_properties_value_constraints_annotated(
    output_file: Path,
    output_model_type: DataModelType,
    expected_file: str,
) -> None:
    ...
    match output_model_type:
        case DataModelType.PydanticV2BaseModel | DataModelType.PydanticV2Dataclass:
            ...
            assert_generated_model_json_validation(
                ...
                invalid_json=_ADDITIONAL_PROPERTIES_CONSTRAINTS_VALID_JSON.replace(
                    '"nestedMap":{"ok":1}', '"nestedMap":{"bad":0}'
                ),
                expected_error_type="greater_than_equal",
                expected_attribute_path=("nestedMap", "ok"),
                expected_attribute_value=1,
            )
            assert_generated_model_json_validation(
                output_file,
                module_name=f"{module_name}_titled_ref",
                model_name="Payload",
                valid_json=_ADDITIONAL_PROPERTIES_CONSTRAINTS_VALID_JSON,
                invalid_json=_ADDITIONAL_PROPERTIES_CONSTRAINTS_VALID_JSON.replace(
                    '"titledRefMap":{"ok":2}', '"titledRefMap":{"bad":0}'
                ),
                expected_error_type="greater_than_equal",
                expected_attribute_path=("titledRefMap", "ok"),
                expected_attribute_value=2,
            )
            assert_generated_model_json_validation(
                output_file,
                module_name=f"{module_name}_local_title_ref",
                model_name="Payload",
                valid_json=_ADDITIONAL_PROPERTIES_CONSTRAINTS_VALID_JSON,
                invalid_json=_ADDITIONAL_PROPERTIES_CONSTRAINTS_VALID_JSON.replace(
                    '"localTitleRefMap":{"ok":2}', '"localTitleRefMap":{"bad":10}'
                ),
                expected_error_type="less_than_equal",
                expected_attribute_path=("localTitleRefMap", "ok"),
                expected_attribute_value=2,
            )
            assert_generated_model_json_invalid(
                output_file,
                module_name=f"{module_name}_root",
                model_name="Payload",
                invalid_json=_ADDITIONAL_PROPERTIES_CONSTRAINTS_VALID_JSON.replace(
                    '"rootMap":{"ok":2}', '"rootMap":{"bad":10}'
                ),
                expected_error_type="less_than_equal",
            )
            invalid_cases = (
                (
                    '"nullableValueMap":{"nil":null,"ok":1}',
                    '"nullableValueMap":null',
                    "dict_type",
                ),
                (
                    '"closedArrayValueMap":{"empty":[]}',
                    '"closedArrayValueMap":{"empty":[1]}',
                    "too_long",
                ),
                (
                    '"allOfValueMap":{"ok":5}',
                    '"allOfValueMap":{"bad":10}',
                    "less_than_equal",
                ),
                (
                    '"titledAllOfValueMap":{"ok":5}',
                    '"titledAllOfValueMap":{"bad":10}',
                    "less_than_equal",
                ),
                (
                    '"nullableAllOfValueMap":{"nil":null,"ok":5}',
                    '"nullableAllOfValueMap":{"bad":10}',
                    "less_than_equal",
                ),
                (
                    '"unionValueMap":{"int":5,"text":"good"}',
                    '"unionValueMap":{"int":5,"text":"x"}',
                    "int_parsing",
                ),
                (
                    '"objectValueMap":{"entry":{"key":"value"}}',
                    '"objectValueMap":{"entry":{}}',
                    "too_short",
                ),
                (
                    '"modeledValueMap":{"entry":{"value":1}}',
            ...
            for index, (valid_fragment, invalid_fragment, expected_error_type) in enumerate(invalid_cases):
                assert_generated_model_json_invalid(
                    output_file,
                    module_name=f"{module_name}_edge_{index}",
                    model_name="Payload",
                    invalid_json=_ADDITIONAL_PROPERTIES_CONSTRAINTS_VALID_JSON.replace(
            ...
            if output_model_type is DataModelType.PydanticV2BaseModel:
                assert_generated_model_json_invalid(
                    output_file,
                    module_name=f"{module_name}_extra",
                    model_name="Payload",
                    invalid_json=_ADDITIONAL_PROPERTIES_CONSTRAINTS_VALID_JSON.replace('"extra":3', '"extra":10'),
                    expected_error_type="less_than_equal",
                )
            return
        case DataModelType.MsgspecStruct:
            ...
            with _generated_model(output_file, "additional_properties_constraints_msgspec", "Payload") as model:
                ...
                invalid_fragments = (
                    ...
                    ('"titledRefMap":{"ok":2}', '"titledRefMap":{"bad":0}'),
                    ('"localTitleRefMap":{"ok":2}', '"localTitleRefMap":{"bad":10}'),
                    ('"closedArrayValueMap":{"empty":[]}', '"closedArrayValueMap":{"empty":[1]}'),
                    ('"allOfValueMap":{"ok":5}', '"allOfValueMap":{"bad":10}'),
                    ('"titledAllOfValueMap":{"ok":5}', '"titledAllOfValueMap":{"bad":10}'),
                    (
                        '"nullableAllOfValueMap":{"nil":null,"ok":5}',
                        '"nullableAllOfValueMap":{"bad":10}',
                    ),
                ...
                for valid_fragment, invalid_fragment in invalid_fragments:
                    with pytest.raises(msgspec.ValidationError):
                        msgspec.json.decode(
                            _ADDITIONAL_PROPERTIES_CONSTRAINTS_VALID_JSON.replace(
                                valid_fragment, invalid_fragment
                            ).encode(),
                            type=model,
                        )
                with pytest.raises(msgspec.ValidationError):
                    msgspec.json.decode(
                        _ADDITIONAL_PROPERTIES_CONSTRAINTS_VALID_JSON.replace(
                            '"nullableValueMap":{"nil":null,"ok":1}', '"nullableValueMap":null'
                        ).encode(),
                        type=model,
                    )
            return
...
def test_main_jsonschema_additional_properties_value_constraints_schema_validators(output_file: Path) -> None:
    ...
    assert_generated_model_json_validation(
        output_file,
        module_name="additional_properties_constraints_schema_validators",
        model_name="Payload",
        valid_json=valid_json,
        invalid_json=valid_json.replace('"extra":3', '"extra":0'),
        expected_error_type="greater_than_equal",
        expected_attribute_path=("nestedMap", "ok"),
        expected_attribute_value=1,
    )
...
def test_main_jsonschema_generate_schema_validators_not_required(
    output_file: Path,
) -> None:
    """Generate and execute a model whose not keyword forbids two properties together."""
    run_main_and_assert(
        input_path=JSON_SCHEMA_DATA_PATH / "schema_validators_not_required.json",
        output_path=output_file,
        input_file_type="jsonschema",
        extra_args=[
            "--output-model-type",
    ...
    assert_generated_model_json_validation(
        output_file,
        module_name="output_not_required",
        model_name="NotRequired",
        valid_json='{"rules":["a"]}',
        invalid_json='{"rules":["a"],"signals":["b"]}',
        expected_error_type="value_error",
        expected_attribute_path=("rules",),
        expected_attribute_value=["a"],
    )


def test_main_jsonschema_generate_schema_validators_conditional_presence(
    output_file: Path,
) -> None:
    """Generate and execute a model whose if condition only requires a property to be present."""
    run_main_and_assert(
        input_path=JSON_SCHEMA_DATA_PATH / "schema_validators_conditional_presence.json",
        output_path=output_file,
        input_file_type="jsonschema",
        extra_args=[
            "--output-model-type",
    ...
    assert_generated_model_json_validation(
        output_file,
        module_name="output_conditional_presence",
        model_name="ConditionalPresence",
...
@pytest.mark.parametrize(
    ("schema", "frozen"),
    [
        ("unique_items_enum_set", False),
        ("unique_model_sets/frozen_any", True),
        ("unique_model_sets/frozen_list", True),
        ("unique_model_sets/nested", False),
        ("unique_model_sets/nested", True),
    ],
)
def test_unique_items_model_set_generation(output_file: Path, schema: str, frozen: bool) -> None:
    """Generate requested sets without imposing frozen or field-type requirements."""
    run_main_and_assert(
        input_path=JSON_SCHEMA_DATA_PATH / f"{schema}.json",
        output_path=output_file,
        input_file_type="jsonschema",
        assert_func=assert_file_content,
        expected_file=f"{schema}{'_frozen' if frozen else ''}.py",
        extra_args=[
            "--output-model-type",
            "pydantic_v2.BaseModel",
            "--use-unique-items-as-set",
    ...
    for valid_path in sorted(payloads.glob("*_valid.txt")):
        ...
        for container in (dict, UserDict, MappingProxyType):
            ...
            with _generated_model(
                ...
                assert_output(
                    json.dumps(value.model_dump(exclude_unset=True), default=dict, indent=2) + "\n", valid_path
                )
                _assert_model_json_invalid(
                    model.model_validate, invalid, "int_parsing" if case == "Pattern" else "value_error"
                )
...
RESOURCES_CASES = json.loads((RESOURCES_EXPECTED / "cases.json").read_text(encoding="utf-8"))
...
@pytest.mark.parametrize(
    ...
    [
        *(
            ...
            for entrypoint in ("cli", "api", "dict", "custom-cli", "custom-api")
        ),
        *(
            (case, entrypoint)
...
def test_nested_schema_resources(tmp_path: Path, case: str, entrypoint: str, formatter: str) -> None:  # noqa: PLR0914
    ...
    registry = Registry().with_resources(
        ...
        for relative in RESOURCES_CASES[case]["resources"]
    )
    validator = validator_class(schema, registry=registry)
    output = tmp_path / "output.py"
    expected_file = (
        RESOURCES_EXPECTED / "custom" if entrypoint.startswith("custom-") else RESOURCES_EXPECTED
    ) / f"{case}.py"
    formatters = ["builtin"] if formatter == "builtin" else ["black", "isort"]
    requests = []
    with ExitStack() as stack:
        ...
        with (
            ...
            if entrypoint.endswith("cli"):
                ...
                run_generate_and_assert(
                    input_=schema if entrypoint == "dict" else input_value,
                    expected_file=expected_file,
        ...
        if entrypoint.startswith("url-"):
            assert_output(json.dumps(requests, indent=2), RESOURCES_EXPECTED / f"{case}_requests.txt")
    records = {"native": [], "generated": []}
    with _generated_model(output, "generated_nested_resource", "Root") as model:
        for payload in RESOURCES_CASES[case]["payloads"]:
...
def test_missing_embedded_anchor(
    ...
    payload = json.loads((RESOURCES_EXPECTED / "missing_anchor_payload.json").read_text(encoding="utf-8"))
    with pytest.raises(Unresolvable) as native_error:
        validator.is_valid(payload)
    assert_output(str(native_error.value), RESOURCES_EXPECTED / "missing_anchor_native_error.txt")
    output = tmp_path / "output.py"
    formatters = ["builtin"] if formatter == "builtin" else ["black", "isort"]
    requests = []
    with ExitStack() as stack:
        ...
        with assert_inputs_not_mutated({"schema": schema}):
            if entrypoint.endswith("cli"):
                run_main_with_args(
                    ...
                    expected_exit=Exit.ERROR,
                    capsys=capsys,
                    expected_stderr=(RESOURCES_EXPECTED / "missing_anchor_cli_error.txt").read_text(encoding="utf-8"),
                )
            else:
                with pytest.raises(Error) as error:
                    run_generate_and_assert(
                        input_=schema if entrypoint == "dict" else input_value,
                        expected_file=RESOURCES_EXPECTED / "missing_anchor_api_error.txt",
                        config=GenerateConfig(
                            input_file_type=InputFileType.JsonSchema,
                            output=output,
                            allow_remote_refs=entrypoint.startswith("url-"),
                ...
                assert_output(str(error.value), RESOURCES_EXPECTED / "missing_anchor_api_error.txt")
        if entrypoint.startswith("url-"):
            assert_output(json.dumps(requests, indent=2), RESOURCES_EXPECTED / "missing_anchor_requests.txt")
...
def test_repeated_schema_documents(tmp_path: Path, formatter: str) -> None:
    ...
    assert_output(json.dumps([list(path) for path in generated]), RESOURCES_EXPECTED / "duplicate_document_paths.txt")
    for name in ("first", "second"):
        result = generated[f"{name}.py",]
        assert_output(result.body, RESOURCES_EXPECTED / "duplicate_documents" / f"{name}.py")
...
def test_explicit_frozen_set_hashes(
    ...
    with _generated_model(output, "explicit_set_output", "Container") as container:
        ...
        if not case["safe"]:
            ...
            return
        originals = [
            MODELS[native_name].model_validate(payload),
            MODELS[native_name].model_validate(payload),
        ]
        runtime.update(
            field_order=list(item.model_fields) == list(MODELS[native_name].model_fields),
            ...
            == MODELS[native_name].model_validate_json(json.dumps(payload)).model_dump(mode="json"),
        )
        values = (
            container.model_validate({"items": [payload, payload]}).items,
            TypeAdapter(set[type(originals[0])]).validate_python([payload, payload]),
        )
        runtime.update(
            pairs=[
                {
            ...
            generated_size=len(values[0]),
            native_size=len(values[1]),
            container_dump=[x.model_dump(mode="json") for x in values[0]]
            == [x.model_dump(mode="json") for x in values[1]],
        )
        assert_output(
            json.dumps(runtime, indent=2) + "\n",
        ...
        with pytest.raises(ValidationError):
            MODELS[native_name].model_validate_json(
                (DATA_PATH / "payloads/explicit_frozen_sets/invalid.json").read_text()
            )
        with pytest.raises(ValidationError):
            item.model_validate({})
        with pytest.raises(ValidationError):
            MODELS[native_name].model_validate({})
...
ROOT_ALIAS_LEGACY_API_OUTPUTS = {("plain_1_1_0.py", True): "plain_1_1_0_black23.py"}
