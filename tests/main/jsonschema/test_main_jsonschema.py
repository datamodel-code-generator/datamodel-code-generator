...
def assert_schema_required_group_validator_not_generated(
    output_file: Path,
    expected_name: str | Path | None = None,  # ruff: ignore[unused-function-argument]
    encoding: str = "utf-8",
    transform: object | None = None,  # ruff: ignore[unused-function-argument]
) -> None:
...
def test_main_jsonschema_empty_field_name(  # ruff: ignore[too-many-branches]
    output_file: Path,
...
def test_field_name_bindings(
    ...
    with _generated_model(output_file, "field_name_binding_model", "Record") as model:
        ...
        if "items" in properties:
            ...
            # Union caching can reuse equivalent Literal arguments in a different order.
            observations["choice_literals"] = sorted(
                get_args(next(arg for arg in get_args(hints["choice"]) if get_args(arg))),
                key=properties["choice"]["enum"].index,
            )
...
def test_nested_schema_resources(tmp_path: Path, case: str, entrypoint: str, formatter: str) -> None:  # ruff: ignore[too-many-locals]
